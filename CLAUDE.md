...
- **`Ok(())` from an injection is not evidence the text landed.** macOS Secure Input drops synthetic keystrokes while reporting success, so `type_text` forgets the record instead of appending to it whenever `secure_input_active()`. A record that claims text is on screen when it is not is worse than no record: the next `replace_last` backspaces over the fiction and into the user's own writing.
- **The backspace count is bounded by what we recorded — that is a bound on the count, not a promise about whose characters go.** The clamp lives in exactly one place (`unicode::char_index_from_end`); a second one elsewhere would hide a broken first from the tests. Where our text *begins* is only a real cluster boundary if the character in front of it does not join ours, and three UAX #29 rules join across a seam the crate cannot see: GB6/GB7 (Hangul jamo), GB11 (emoji ZWJ), GB12/GB13 (regional indicators). Only the context-free joins (GB9/GB9a) are handled, by `plan::joins_the_cluster_before`. **#76** threads the preceding text in and deletes the special case; do not extend the probe instead.
- **A replace lifts held modifiers first.** Ctrl+Backspace deletes a word, so the PTT bug class is worse for a backspace run than for a type run. Every emitting plan starts with `Action::LiftModifiers`, and `a_replace_never_leaves_a_latched_modifier_behind` is the test that keeps it there.

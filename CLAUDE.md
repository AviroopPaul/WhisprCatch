...
- **The release pass is `stream::plan_release`, and it is pure.** Live typing shows the model's raw words; cleanup only runs on the finished transcript, and no transform is prefix-stable, so appending a tail to text that is already wrong cannot spell the right answer (#50). `plan_release` appends when the finished text still begins with what is on screen and otherwise replaces the whole streamed run via `Injector::replace_last`. It takes what is on screen as a string, not a state machine, so every case is a unit test with no audio, model or display server.
  - **`Stream::typed` is what reached the screen, never `committed.join(" ")`.** A delta is committed before it is injected, so a failed injection leaves a committed word that is not there. Only `mark_typed` after a successful send.
  - **A replace is refused, not attempted, when it cannot be trusted**: the injector's record must still cover the run (it is dropped on any injection error or Secure Input), and the sink must be able to lift a held modifier. Both fall back to appending — wrong in the direction that only ever adds characters.
  - **Live typing is gated at press time, not warned about.** `live_typing_enabled` switches it off when cleanup can rewrite and the display server cannot take text back, so the broken combination is unreachable rather than discouraged. The user-visible half is the "Live typing" description in Settings; keep the two in step.
  - **Known and undetectable**: if the user types manually between the last streaming pass and release, the replace eats their keystrokes. Bounded by `Typed` and by forgetting the record at the start of each utterance. Detecting it would mean watching every key, which this project will not do.
...
- **`prefix_stable()` is a promise to the streaming loop**, not a label. It means `apply(prefix)` is a prefix of `apply(whole)` — much stronger than "never shortens the text", because a streaming pass has already typed `apply(prefix)`. **All six return `false`**, each with a counterexample in its own module: a substitution done "in place" still breaks the property whenever its trigger straddles the streaming boundary ("twenty" → `20`, then "twenty five" → `25`). That is why the release pass reconciles by *replacing* what streamed rather than by running a prefix-stable subset (#50), and why `apply_prefix_stable` still has no production caller. Returning `true` needs a proof against `prefix_violation` in `testing.rs`, not an intuition.
...
- **Where the lift cannot work, the planner declines rather than the caller checking (#77).** On Wayland `lift_modifiers` reports success having lifted nothing, so `Capabilities::can_lift_modifiers` is false there and `Plan::replace` returns a *declined* plan — no actions at all, never a half-replace that types the new text on top of the old. `is_declined()` is distinct from `is_empty()`: nothing-to-do is success, refusal is an error out of `replace_last`. `Injector::can_replace` (and `can_replace_typed_text()` for processes with no injector) lets a caller with a non-destructive alternative ask first. Typing never declines — losing keystrokes to shortcuts is bad, deleting the user's words is unrecoverable.
- **There is no clipboard path.** The 200-char paste threshold is planned and tested, but no backend sets `Capabilities::paste`, so `Action::Paste` is never emitted. Implementing it means clobbering the user's clipboard and synthesising ⌘V/Ctrl+V — see #68 before starting.

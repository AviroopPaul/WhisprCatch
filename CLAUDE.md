...
- **`append_safe()` is a promise to the streaming loop**, not a label. Live passes may only run transforms that never shorten or reorder text, because words already typed cannot be taken back until injector replace (#41) lands.
